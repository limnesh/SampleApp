...
export class LocationPage {
  ...
  addGPSAddress( address :any) : any {
	  
	  this.showToast('Location Searching!');
				...
				this.geolocation.getCurrentPosition(locationOptions).then(
				  (position) => {
					this.loading.dismiss().then(() => {
					  ...
					  this.custService.addAddress(this.authService.getLoggedUID(),address); 
  ...
  saveMapLocation() {
	  this.addGPSAddress(null);
	  let alert = this.alertCtrl.create({
            title: 'Location',
            message: 'this.custService.deliveryAddresses' ,
			 buttons: [
              {
                text: 'Cancel'
              }]
          ...
          alert.present();
		//this.navParams.set('thing1','Hello');
  ...
  initializeMap() {
    this.zone.run(() => {
      ...
      this.map = new google.maps.Map(mapEle, {
        zoom: 14,
        center: { lat: 50.563223, lng: 26.226058 },
